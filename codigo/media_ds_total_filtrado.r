...
library(ggpubr)
# Se carga la tabla filtrada y se guarda en la varible mydata
mydata <- read.table("../cultivos_nacional_filtrado.csv", header=TRUE, sep=",");
...
medias_por_a = mydata %>% 
group_by(P_S6P47B) %>%
summarise(media_area_cosechada = mean(AREA_COSECHADA),
          media_area_sembrada = mean(AREA_SEMBRADA))
 
destan_por_a = mydata %>% 
group_by(P_S6P47B) %>%
summarise(ds_area_cosechada = sd(AREA_COSECHADA),
          ds_area_sembrada = sd(AREA_SEMBRADA))

# Se unen las tablas de media y desviacion estandar para luego graficarlas
